...
- **Inside a JS template literal `////` is not a comment — it is text, and the user READS it on screen.** It happened: two markers landed inside frappe's form hero and the sentence appeared under the amount of every document (neoffice-maintenance#331). The file still parsed and the tests still passed. When the anchor line is inside a `` ` … ` ``: put the marker **above the statement that opens the literal**, or use `<!-- … -->` when the literal builds HTML, or skip that anchor and say so — never write text that will be displayed. `fork_markers.py verify` now refuses it, so a run that does it comes back red.
